                            ...
                            if (keyCount > 0) {
                                ...
                                for (var key in json) {
                                    if (Object.prototype.hasOwnProperty.call(json, key)) {
                                        key_esc = htmlEscape(key);
                                        var keyRepr = options.withQuotes ?
                                            '<span class="json-string">"' + key_esc + '"</span>' : key_esc;
...
def create_result_page(data):
    replaced_HTML = re.sub('#insert_data_here#', json.dumps(data), HTML)

    f = open(f"{path}/result.html", "w")
    f.write(replaced_HTML)
    f.close()
    print("Open HTML file to see results!")
